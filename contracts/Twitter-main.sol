...
contract Twitter is Ownable {
    ...
    modifier onlyRegistered() {
        require(
            bytes(profileContract.getProfile(msg.sender).username).length > 0,
            "USER NOT REGISTERED"
        );
        _;
    }
    ...
    function likeTweet(address auther, uint256 _id) external onlyRegistered {
    ...
    function unLikeTweet(address auther, uint256 _id) external onlyRegistered {
        require(tweets[auther][_id].id == _id, "Tweet does not exist");
